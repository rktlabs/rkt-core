...
import { LinearBondingCurveAMM, MintService } from '../../..'
import {
    AssetRepository,
    PortfolioRepository,
...
import { BondingCurveAMMParams } from '../bondingCurveAMM/bondingCurveAMM'

const FieldValue = admin.firestore.FieldValue
const logger = log4js.getLogger('BondingCurveAMMParams')

export type KMakerAMMSettings = {
    initialUnits?: number
    initialValue?: number
    initialPrice?: number
...
export class KMaker extends MarketMakerServiceBase {
    ...
    private processOrderSize(signedTakerOrderSize: number): TMakerResult | null {
        const makerParams = this.marketMaker.params as KMakerParams
        if (!makerParams) {
            return null
        }
        const { propsUpdate } = this.computePropsUpdate(makerParams, signedTakerOrderSize)
    ...
    private computePropsUpdate(marketMaker: KMakerParams, orderSize: number) {
        const initialPoolUnits = marketMaker.poolUnits
        const initialPoolCoins = marketMaker.poolCoins
        // const k = marketMaker.k
    ...
    private computeInitialState(settings: KMakerAMMSettings): KMakerParams {
        const initMadeUnits = settings?.initialUnits || 0
        const initPrice = settings?.initialPrice || 1
        const initialPoolUnits = settings?.initialPoolUnits || 1000
        const poolUnits = initialPoolUnits - initMadeUnits
        const poolCoins = poolUnits * initPrice

        const makerState: KMakerParams = {
