...
class PortfolioOrderService {
    constructor(assetRepository, portfolioRepository, transactionRepository, marketMakerRepository) {
        ...
        // PRIVATE
        ////////////////////////////////////////////////////////
        this._generateExchangeOrder = (order) => {
            const exchangeOrder = {
                operation: 'order',
    ...
    submitNewPortfolioOrderAsync(orderPayload) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const exchangeOrder = this._generateExchangeOrder(newPortfolioOrder);
    ...
    unwindOrder(portfolioId, orderId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const exchangeOrder = this._generateExchangeOrder(newOrder);
            // if (this.eventPublisher) {
            //     await this.eventPublisher.publishExchangeOrderCreateAsync(exchangeOrder, 'orderHandler')
            // }
            return newOrder;
    ...
    cancelOrder(portfolioId, orderId) {
        return __awaiter(this, void 0, void 0, function* () {
            const order = yield this.portfolioOrderRepository.getDetailAsync(portfolioId, orderId);
            if (order) {
                const exchangeOrder = this._generateCancelExchangeOrder(order);
                // update state to 'cancelPending'
                const patch = { status: 'cancelPending' };
                const updatedOrder = yield this.portfolioOrderRepository.updateAsync(portfolioId, orderId, patch);
                return updatedOrder;
            }
            else {
                return undefined;
    ...
    _generateCancelExchangeOrder(order) {
        const exchangeOrder = {
            operation: 'cancel',
            assetId: order.assetId,
            portfolioId: order.portfolioId,
