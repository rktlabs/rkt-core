...
class ExchangeService {
    constructor(assetRepository, portfolioRepository, eventPublisher) {
        ////////////////////////////////////////////////////
        //  onTrade
        //  - store trade
        ...
        this.onTrade = (trade) => __awaiter(this, void 0, void 0, function* () {
            ...
            this.orderNotificationPublisher.publishOrderEventCompleteAsync(trade.taker.portfolioId, trade.taker.orderId, trade.tradeId, 'marketMaker'); // async - don't wait to finish
        });
        ////////////////////////////////////////////////////
        //  onUpdateQuote
        ...
        this.transactionService = new __1.TransactionService(assetRepository, portfolioRepository);
        this.marketMakerService = new __1.MarketMakerService(assetRepository, portfolioRepository);
    }
    ////////////////////////////////////////////////////
            ...
            try {
                ...
                if (marketMaker) {
                    if (orderSide === 'bid') {
                        ...
                        yield this.verifyFundsAsync(portfolioId, assetId, orderSide, orderSize, currentPrice);
                    }
                    ////////////////////////////////////
                ...
                if (exchangeOrder && exchangeOrder.status === 'received') {
                    ...
                    this.orderNotificationPublisher.publishOrderEventFailedAsync(exchangeOrder.portfolioId, exchangeOrder.orderId, reason, 'marketMaker'); // async - don't wait to finish
                }
                throw error;
            }
    ...
    onFill(taker) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.orderNotificationPublisher.publishOrderEventFillAsync(portfolioId, orderId, filledSize, filledValue, filledPrice, makerRemaining, 'marketMaker'); // async - don't wait to finish
    ...
    verifyFundsAsync(portfolioId, assetId, orderSide, orderSize, currentPrice = 0) {
        return __awaiter(this, void 0, void 0, function* () {
            // verify that portfolio exists.
            const portfolio = yield this.portfolioRepository.getDetailAsync(portfolioId);
            if (!portfolio) {
